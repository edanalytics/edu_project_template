...
with stg_discipline_actions as (
    ...
    where actual_discipline_action_length is not null
),
calendar_dates as (
...
discipline_start_day_of_school_year as (
    select da.k_student, da.k_school__responsibility, da.school_year,
        ...
        ceiling(da.actual_discipline_action_length) as actual_discipline_action_length,
        ssa.calendar_code,
...
/* Actual Discipline End Date must be an Instructional Day. */
select da.k_student, da.k_school__responsibility, da.school_year,
    da.discipline_action_id, da.discipline_date, da.responsibility_school_id,
    da.student_unique_id,
    {{ error_code }} as error_code,
    concat('Actual Discipline Action Length produces an end date that does not fall on an Instructional Day of the School Calendar used by this Student. Discipline Date: ',
        da.discipline_date, ', Actual Discipline Action Length: ', da.actual_discipline_action_length, '.') as error,
...
where not exists (
        ...
        where cd.k_school = da.k_school__responsibility
            ...
            and cd.day_of_school_year = ((da.day_of_school_year-1) + da.actual_discipline_action_length)
    )